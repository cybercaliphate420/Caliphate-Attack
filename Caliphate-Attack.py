...
def check_intr():
    try:
        ...
        sys.exit(2)
filepath = os.path.isfile('/data/data/com.termux/files/usr/etc/isdcodes.json')
if filepath == True:
	pass
else:
	os.system('apt install wget && wget https://raw.githubusercontent.com/TheSpeedX/TBomb/master/isdcodes.json -O /data/data/com.termux/files/usr/etc/isdcodes.json')
lic = """
...
╱╱╱╱╭━╯┃╱╱╱╱╱╱╱╱╱╱╱╱╱╱╱╱╱╱╱┃┃
╱╱╱╱╰━━╯╱╱╱╱╱╱╱╱╱╱╱╱╱╱╱╱╱╱╱╰╯
                {C}Coder  : {Y}Cyber Caliphate
{Fore.MAGENTA}                Version : {G}{version}
 """
os.system('clear')
...
def readisdc():
    with open("/data/data/com.termux/files/usr/etc/isdcodes.json") as file:
...
def pretty_print(cc, target, success, failed):
    ...
    mesgdcrt.GeneralMessage(f"{G}Target       : " + cc + " " + target)
